$(function() {
  ...
  $("#how-it-cost-js").on("click", function() {
    $(".main-content, #overlay, .main-test-block").slideToggle();
  ...
  $(".onlyDigits").keyup(validateMaxLength);

  $("#pool--length").on("change", function() {
    var maxVal = 8,
      minVal = 1;
    minMaxVal($(this), maxVal, minVal);
  });
  $("#pool--width").on("change", function() {
    var maxVal = 4,
      minVal = 1;
    minMaxVal($(this), maxVal, minVal);
  });
  $("#pool-height").on("change", function() {
    var maxVal = 3.6,
      minVal = 1.8;
    minMaxVal($(this), maxVal, minVal);
  });
...
function minMaxVal(inp, maxVal, minVal) {
  var inpVal = inp.val();
  if (inpVal > maxVal) {
    inp.val(maxVal);
  } else if (inpVal < minVal) {
    inp.val(minVal);
  }
}
